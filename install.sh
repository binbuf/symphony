...
cp "$here/symphony" "$here/symphony.ps1" "$here/symphony.cmd" "$here/README.md" "$here/symphony.config.example.json" "$dest/"
chmod +x "$dest/symphony" 2>/dev/null || true
version="$(node -e "process.stdout.write(String(JSON.parse(require('fs').readFileSync(process.argv[1],'utf8')).version))" "$here/package.json")"

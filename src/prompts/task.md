...
- {progress} is the shared notebook for the whole run; its current content is inlined below. Before you finish, append a section "## {taskId} — {taskTitle}" with what later tasks need to know: real paths, commands that work, contract deviations, gotchas. Facts, not narrative. Never delete other sections. If a later session will need to continue this task, say exactly what remains.

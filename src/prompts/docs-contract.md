...
{roadmap}
   ...
   - Ids are T01, T02, … zero-padded, unique and increasing across the whole file. A task that was broken down by `symphony split` has letter-suffixed children in its place (T10 becomes T10a, T10b, …) — write them the same way as any other task. Every task bullet starts with "- [ ] " (or "- [x] " if that work is already done). No sub-bullets under task bullets. No other bullets or numbered items anywhere in the file that begin with a number or a T-number; put prose in paragraphs.
...
{progressHeader}
   ...
   The harness also maintains {index}, a generated map of the design docs and source files, and a short "Key facts" digest inside {progress} (between the "symphony:digest" HTML comments). Do not create or edit either by hand.

...
export function buildPreparePrompt(ctx: RunContext, report: LintReport): string {
  ...
  return `You are preparing the "${basename(paths.root)}" project for the symphony harness, which will later drive an autonomous coding agent through .docs/ROADMAP.md, one task per session. Your only job in this session is to bring the project's planning documents into the exact layout and format below. Do not write or change application code, do not start any task, do not run the harness. Nobody can answer questions: make reasonable calls and record each one in .docs/PROGRESS.md under a "## Preparation notes" section.
...
## Planning documents outside .docs/
${report.candidates.length ? report.candidates.map((c) => `- ${c}`).join('\n') : '(none)'}
${report.candidates.length ? 'Fold their content into .docs/ (roadmap tasks, task files, design docs, ADRs). Move with `git mv` so history is kept; if other files link to the old path, leave a one-line pointer there. Leave genuine end-user documentation (README, API docs) where it is.' : ''}
...
- Every roadmap task gets a task file at .docs/tasks/NN-<slug>.md from the template, filled with what the sources say; put open questions under "Design notes" as explicit assumptions rather than guessing silently.
...
export async function prepareCommand(ctx: RunContext, opts: { dryRun: boolean }): Promise<number> {
  ...
  const before = lintDocs(paths);
  log.plain('--- lint (before)');
  formatLint(before).forEach((l) => log.plain(l));

  if (!existsSync(paths.docs) && before.candidates.length === 0) {
    log.error('nothing to prepare from: no .docs/ and no planning documents found. Run `symphony init` and write .docs/ROADMAP.md, or paste the output of `symphony brief` to your LLM.');
    return 2;
  }
  ...
  let report = before;
  if (!opts.dryRun) {
    const created = scaffoldDocs(paths, { roadmap: false, config: false });
    created.forEach((p) => log.info(`created ${relative(paths.root, p)}`));
    const added = ensureGitignore(paths.root, ['.symphony/']);
    if (added.length) log.info(`added ${added.join(', ')} to .gitignore`);
    if (created.length || added.length) report = lintDocs(paths);
  }
  if (report.ok) {
    log.info(`prepare: .docs/ is already in the expected format (${report.taskCount} task${report.taskCount === 1 ? '' : 's'}); nothing for the agent to do`);
    return 0;
  }
  ...
  try {
    ...
    log.info(`=== prepare: repairing .docs/ with ${spec.providerName} (${report.findings.filter((x) => x.level === 'error').length} errors, ${report.candidates.length} outside documents)`);
    ...
    if (out.interrupted || ctx.interrupted) { log.warn('prepare interrupted; .docs/ may be half-converted (check git status)'); return 130; }
    ...
    const after = lintDocs(paths);
    log.plain('--- lint (after)');
    formatLint(after).forEach((l) => log.plain(l));
    const commit = commitAll(paths.root, 'docs: normalise .docs for symphony [prepare]');
    log.info(`prepare: git ${describeCommit(commit)}${out.costUsd !== undefined ? ` · $${out.costUsd.toFixed(2)}` : ''}`);
    if (!after.ok) { log.error('.docs/ is still not in the expected format; fix the ✗ items by hand or run `symphony prepare` again'); return 2; }
    return 0;
  } finally {
    releaseLock(paths);
  }

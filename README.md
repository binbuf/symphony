...
It is asked for **two to four short sentences**, told to begin directly with the observation rather than a preamble like "I looked into…" (a stray conversational opener is stripped before display), not to narrate raw status or timing, never to hedge that it is too early to tell, and never to pad. Silence is not the default: it is asked to give its read almost every check and to reply `NO_UPDATE` only at the very start of a run, when the first task is still in flight and nothing has finished. If it does go silent with no earlier summary to keep, the panel shows a dim `No update yet — nothing to interpret until the first task lands.` placeholder.
